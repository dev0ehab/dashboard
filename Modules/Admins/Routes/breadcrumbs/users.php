...
Breadcrumbs::for('dashboard.users.index', function ($breadcrumb) {
    ...
    $breadcrumb->push(trans('admins::user.plural'), route('dashboard.users.index'));
});

Breadcrumbs::for('dashboard.users.create', function ($breadcrumb) {
    $breadcrumb->parent('dashboard.users.index');
    $breadcrumb->push(trans('admins::user.actions.create'), route('dashboard.users.create'));
});
...
Breadcrumbs::for('dashboard.users.edit', function ($breadcrumb, $user) {
    ...
    $breadcrumb->push(trans('admins::user.actions.edit'), route('dashboard.users.edit', $user));
});

Breadcrumbs::for('dashboard.users.trashed', function ($breadcrumb) {
    $breadcrumb->parent('dashboard.users.index');
    $breadcrumb->push(trans('admins::user.trashedPlural'), route('dashboard.users.trashed'));
});
