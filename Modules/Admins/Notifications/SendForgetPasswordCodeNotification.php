...
class SendForgetPasswordCodeNotification extends Notification
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->greeting(trans('admins::auth.emails.forget-password.greeting', [
                'user' => $notifiable->name,
            ]))
            ->subject(trans('admins::auth.emails.forget-password.subject'))
            ->line(trans('admins::auth.emails.forget-password.line', [
                'code' => $this->code,
                'minutes' => ResetPasswordCode::EXPIRE_DURATION / 60,
            ]))
            ->line(trans('admins::auth.emails.forget-password.footer'))
