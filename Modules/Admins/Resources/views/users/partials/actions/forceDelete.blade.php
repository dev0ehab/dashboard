            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"
                        id="modal-title-{{ $user->id }}">@lang('admins::user.dialogs.force.title')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    @lang('admins::user.dialogs.force.info')
                </div>
                <div class="modal-footer">
                    {{ BsForm::get(route('dashboard.users.forceDelete', $user)) }}
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">
                        @lang('admins::user.dialogs.force.cancel')
                    </button>
                    <button type="submit" class="btn btn-danger">
                        @lang('admins::user.dialogs.force.confirm')
                    </button>
                    {{ BsForm::close() }}
                </div>
            </div>
